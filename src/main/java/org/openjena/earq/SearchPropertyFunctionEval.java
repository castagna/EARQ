...
public abstract class SearchPropertyFunctionEval extends PropertyFunctionEval {

	private static final Logger LOG = LoggerFactory.getLogger(SearchPropertyFunctionEval.class);
	
	protected SearchPropertyFunctionEval() {
    ...
    public QueryIterator varSubject(Binding binding, Var match, Var score, String searchString, long limit, float scoreLimit, ExecutionContext execCxt) {
        Iterator<Document> iter = getIndexSearcher(execCxt).search(searchString) ;
    ...
    public QueryIterator boundSubject(Binding binding, Node match, Var score, String searchString, long limit, float scoreLimit, ExecutionContext execCxt) {
        Document doc = getIndexSearcher(execCxt).contains(match, searchString) ;
